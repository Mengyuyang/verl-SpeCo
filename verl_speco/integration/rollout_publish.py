...
def release_draft_weights_payload(weights: Any) -> dict[str, Any]:
    """Drop a completed publish payload and reclaim its allocator pages.

    A Ray-backed drafter publish materializes a full CPU state dict in every
    actor-rollout worker. Merely leaving the RPC method drops the Python local
    variable, but jemalloc/glibc may retain the tensor storage pages in that
    long-lived worker. Clear mutable containers first so the tensor objects
    become unreachable before asking the allocator to return unused pages.

    Callers must invoke this only after the downstream weight update has
    finished consuming the payload.
    """

...
exp_name='qwen3_8b_dspark_train_matched_temp1_vllm_npu'
...
PYTHONUNBUFFERED=1 python3 -m verl_speco.main \
    ...
    actor_rollout_ref.rollout.drafter.training.sample_last_n_steps=2 \
    actor_rollout_ref.rollout.drafter.training.step=8 \
    actor_rollout_ref.rollout.drafter.training.batch_size_per_gpu=4 \
    actor_rollout_ref.rollout.drafter.training.lr=5e-6 \

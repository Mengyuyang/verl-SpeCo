...
# Native DSpark dynamic verification requires vllm-ascend#13216 (vLLM 0.26).
# DRAFTER_PATH must already contain confidence_head.proj.{weight,bias} so the
# rollout engine can start. This job keeps training and publishing that head.
project_name='verl_grpo_example_dspark_dynamic_drafter'
exp_name='qwen3_8b_dspark_dynamic_drafter_vllm_npu'
...
PYTHONUNBUFFERED=1 python3 -m verl_speco.main \
    ...
    +actor_rollout_ref.rollout.engine_kwargs.vllm.additional_config.dynamic_spec_config.method_params.initial_verify_budget_per_req=5 \
    +actor_rollout_ref.rollout.engine_kwargs.vllm.additional_config.dynamic_spec_config.method_params.budget_update_interval=50 \
    +actor_rollout_ref.rollout.engine_kwargs.vllm.additional_config.dynamic_spec_config.method_params.budget_threshold=0.7 \

...
def test_vllm_draft_ipc_streams_buckets_and_advances_revision(monkeypatch) -> None:
    import verl_speco.integration.vllm_runtime as runtime
...
def test_vllm_level2_snapshot_revision_mismatch_fails_closed() -> None:
    extension, _, _ = _revision_runtime_extension()
    extension._speco_draft_runtime_revision = 5
    assert extension._speco_prepare_draft_for_sleep(2) > 0
    extension._speco_draft_runtime_revision = 6
...
def test_vllm_dspark_target_sync_updates_only_outer_lm_head(monkeypatch) -> None:
    class Weight:
        shape = (4, 3)
        device = "npu:0"
        dtype = "bf16"
